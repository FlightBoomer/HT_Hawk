...
void	paramLoad(void)
{
	ctrl.pitch.shell.kp = 3;     // 4
	ctrl.pitch.shell.ki = 0.01;  // 0.02
	
	ctrl.pitch.core.kp = 1.40;   // 1.4   1.4
	ctrl.pitch.core.ki = 0.05;   // 0.07  0.45
	ctrl.pitch.core.kd = 0.35;   // 0.35  0.70，这边注意一下，Multirotor_control里内环的微分项我们做了修改，即乘以4(2.5ms->10ms)，这样参数就不怎么要动了
	
	//The data of roll
	ctrl.roll.shell.kp = 3;
	ctrl.roll.shell.ki = 0.01;

	ctrl.roll.core.kp = 1.40;
	ctrl.roll.core.ki = 0.05;
	ctrl.roll.core.kd = 0.35;
	
	//The data of yaw
	ctrl.yaw.shell.kp = 2;    // 5  4
	ctrl.yaw.shell.kd = 0;    // 0  0
	
	ctrl.yaw.core.kp = 1.8;   // 1.8
	ctrl.yaw.core.ki = 0;     // 0
	ctrl.yaw.core.kd = 0.1;   // 0.1
